...
class _CreateSplitBillScreenState extends State<CreateSplitBillScreen> {
  ...
  void dispose() {
    super.dispose();
    Future.delayed(Duration.zero, () {
      provider?.disposeCreateSplitBill();
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: CustomAppBar(title: 'Create Split Bill'),
        ...
        child: Column(
          ...
          children: [
            ...
            CustomTextField(
              ...
              isRequired: true,
              // maxLength: 30,
              onChanged: (_) => provider.checkIfSplitBillIsComplete(),
            ),
            const Gap(20),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      const Gap(10),
                      _ImagePickerCard(
                        selectedFile: provider.billImageFile,
                        onTap: () async {
                          ...
                          if (image != null) {
                            provider.billImageFile = File(image.path);
                ...
                children: [
                  Row(
                    ...
                    children: [
                      ...
                      Switch(
                        value: provider.allowPartialPayments,
                        activeThumbColor: appPrimaryColor,
                        onChanged: (val) {
                          provider.toggleAllowPartialPayments(val);
                        },
                      ),
                    ],
                  ),
                  if (provider.allowPartialPayments) ...[
                    const Gap(16),
                    const Divider(),
                    const Gap(16),
                    CustomTextField(
                      ...
                      hintText: "0.00",
                      controller: provider.minPaymentAmountForPartial,
                      isRequired: true,
                      formatters: MoneyInputFormatter(),
                      textInputType: const TextInputType.numberWithOptions(
                        decimal: true,
                      ...
                      onChanged: (value) {
                        ...
                        //   provider.minPaymentAmountForPartial =
            ...
            CustomButton(
              ...
              onTap: () async {
                ...
                provider.createSplitBill(
                  ...
                  totalAmount:
                      double.tryParse(
                        provider.totalAmountController.text.trim().replaceAll(
                          ",",
                          "",
                        ),
                      ) ??
                    ...
                    children: [
                      ...
                      if (provider.getSelectedParticipants().isNotEmpty) ...[
                        Column(
                          ...
                          children: [
                            Text(
                              "Added (${provider.getSelectedParticipants().length})",
                              style: txStyle13.copyWith(
                                fontWeight: FontWeight.w600,
                                color: appPrimaryColor,
                              ),
...
class ManageBillAmountSheet extends StatefulWidget {
  const ManageBillAmountSheet({super.key});

  @override
  State<ManageBillAmountSheet> createState() => _ManageBillAmountSheetState();
}

class _ManageBillAmountSheetState extends State<ManageBillAmountSheet> {
  ...
  void initState() {
    ...
    Future.delayed(Duration.zero, () {
      provider = Provider.of<NewSplitBillProvider>(context, listen: false);
      if (provider?.getSelectedParticipants().isNotEmpty ?? false) {
  ...
  double _calculateTotalEntered() {
    ...
    for (var participant in provider!.getSelectedParticipants()) {
      final amount =
          double.tryParse(
  ...
  bool _isEquallySplit() {
    if (provider == null || provider!.getSelectedParticipants().isEmpty) {
      return false;
    }
